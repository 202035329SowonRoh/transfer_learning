...
class StereoDataset(Dataset):
    ...
    def transform_fn(sample):
        # 여기서 원하는 데이터 변환을 수행합니다.
        # 예를 들어 이미지를 PyTorch 텐서로 변환할 수 있습니다.
        sample['left'] = to_tensor(sample['left'])
        sample['right'] = to_tensor(sample['right'])

        if 'disp' in sample:
            sample['disp'] = to_tensor(sample['disp'])
...
class Spring(StereoDataset):
    def __init__(self,
                 data_dir='Spring_Dataset',
                 transform=None,
                 ):
        super(Spring, self).__init__(transform=transform, is_spring=True)

        # 데이터셋 파일 경로 수집
        left_files = sorted(glob(data_dir + '/left/*.png'))
        right_files = sorted(glob(data_dir + '/right/*.png'))
        disp_files = sorted(glob(data_dir + '/disp/*.pfm'))  # 적절한 확장자로 변경
...
def build_dataset(args):
    if args.stage == 'sceneflow':
        ...
        spring_dataset = Spring(transform=train_transform)

        return spring_dataset
